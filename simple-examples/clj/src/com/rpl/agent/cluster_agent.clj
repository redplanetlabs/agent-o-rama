   ...
   (node
    ...
    (fn [agent-node {:keys [task-id workers data]}]
      (let [coordination-store (get-store agent-node "$coordination-store")]
        ...
        (store/put! coordination-store task-id
                    {:status "coordinating"
                     :workers workers
                     :data data
   ...
   (node
    ...
    (fn [agent-node {:keys [worker-id task items]}]
      (let [worker-store (get-store agent-node "$worker-store")]
        ...
        (println (str "WorkerAgent-" worker-id ": Starting work on task " task))

         ;; Store worker state
        (store/put! worker-store worker-id
                    {:status "working"
                     :task task
...
(defn -main
  ...
  (with-open [ipc (rtest/create-ipc)]
    ...
    (let [coord-manager (agent-manager ipc (get-module-name ClusterAgentModule))
      ...
      (println "Demonstrating cross-module agent communication and coordination")
      ...
      (let [task-data ["item1" "item2" "item3" "item4" "item5" "item6"]
        ...
        (let [coord-result (agent-invoke coordinator-agent
          ...
          (let [items-per-worker (partition-all (Math/ceil (/ (count task-data) worker-count)) task-data)
                worker-results (atom [])]
            ...
            (let [total-processing-time (reduce + (map #(get-in % [:work-result :processing-time]) @worker-results))
                  total-items-processed (reduce + (map #(get-in % [:work-result :items-processed]) @worker-results))]
              (println "  Total processing time:" total-processing-time "ms")
              (println "  Total items processed by workers:" total-items-processed))))
        ...
        (println "- Cross-module communication enables distributed processing")))))
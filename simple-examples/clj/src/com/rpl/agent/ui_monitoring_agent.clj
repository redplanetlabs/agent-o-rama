    ...
    (aor/node
     ...
     (fn [agent-node {:keys [items batch-size current-batch]}]
       (let [counters        (aor/get-store agent-node "$$counters")
             ...
             processed-count (+ (store/get counters "processed-items")
                                (count batch))]

         (when (seq batch)
           (println (format "Processing batch %d: %d items"
                            (inc current-batch)
                            (count batch)))
  ...
  (with-open [ipc (rtest/create-ipc)]
    ...
    (with-open [ui (aor/start-ui ipc {:port 1974 :no-input-before-close true})]
      (let [manager (aor/agent-manager ipc
          ...
          (let [progress-updates (atom [])]
            ...
            (let [result (aor/agent-result agent invoke)]
              ...
              (println "  Final result:"
                       (select-keys result
                                    [:action :total-processed :total-batches]))
        ...
        (println
         "- Complex agent graphs can be debugged and traced visually")))))

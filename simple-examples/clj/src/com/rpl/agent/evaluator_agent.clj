                             ...
                             (fn [idx output]
                               {:index idx
                                ...
                                :score (+ (count (str output))
                                          (if (.contains (str output) "good") 10 0)
                                          (if (.contains (str output) "bad") -10 0))})
                             outputs)
     ...
     (fn [agent-node _]
       (let [manager (aor/agent-manager agent-node)]
         ...
         (let [length-result (aor/try-evaluator manager
                                                ...
                                                "Expected"
                                                "Short response")
               concise-result (aor/try-evaluator manager
                                                 "concise-30"
                                                 "Test input"
                                                 "Expected"
           ...
           (let [comparison-result (aor/try-comparative-evaluator
             ...
             (let [examples [(aor/mk-example-run "input1" "positive" "positive")
                             (aor/mk-example-run "input2" "negative" "negative") 
                             (aor/mk-example-run "input3" "positive" "negative")
                             (aor/mk-example-run "input4" "positive" "positive")]
...
(defn -main
  ...
  (with-open [ipc (rtest/create-ipc)]
    ...
    (let [manager (aor/agent-manager ipc
      ...
      (println "Creating and testing different types of evaluators for agent assessment")

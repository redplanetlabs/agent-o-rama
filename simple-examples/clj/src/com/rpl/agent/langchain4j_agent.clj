  ...
  (->
    ...
    (aor/node
     ...
     (fn [agent-node {:keys [conversation-history last-response turn-count]}]
       (println (format "Continuing conversation (turn %d)..." (inc turn-count)))
       
       (let [model (aor/get-agent-object agent-node "openai-model")
             follow-up-message (UserMessage. "Can you provide more details or an example?")
    ...
    (aor/node
     "analyze-conversation"
     nil
     (fn [agent-node {:keys [full-conversation initial-response 
                             follow-up-response total-turns]}]
       (println "Analyzing conversation...")
       
       (let [model (aor/get-agent-object agent-node "openai-model")
             ;; Create analysis prompt
             analysis-prompt (str "Summarize this conversation in one sentence: "
  ...
  (if (System/getenv "OPENAI_API_KEY")
    (with-open [ipc (rtest/create-ipc)]
      ...
      (let [manager (aor/agent-manager ipc
                                       (rama/get-module-name LangChain4jAgentModule))
        ...
        (let [result (aor/agent-invoke agent
                                       {:system-prompt "You are a helpful assistant that explains technical concepts clearly and concisely."

  ...
  (-> topology
      ...
      (aor/node
       ...
       (fn [agent-node {:keys [task-name duration]}]
         (println
          (format "Starting task '%s' (will take %d ms)" task-name duration))
         ;; Simulate work
      ...
      (aor/node
       ...
       (fn [agent-node {:keys [task duration started-at]}]
         (let [completed-at    (System/currentTimeMillis)
           ...
           (println (format "Completed task '%s' (actual duration: %d ms)"
                            task
                            actual-duration))
...
(defn -main
  ...
  (with-open [ipc (rtest/create-ipc)]
    ...
    (let [manager (aor/agent-manager ipc
      ...
      (let [task1-invoke (aor/agent-initiate agent
                                             {:task-name "Data Processing"
                                              :duration  1000})
            task2-invoke (aor/agent-initiate agent
                                             {:task-name "Report Generation"
                                              :duration  800})
            task3-invoke (aor/agent-initiate agent
                                             {:task-name "Email Sending"
                                              :duration  500})]
  ...
  (with-open [ipc (rtest/create-ipc)]
    ...
    (let [manager (aor/agent-manager ipc
      ...
      (let [start-time (System/currentTimeMillis)]
        (doseq [i (range 1 4)]
          (aor/agent-invoke agent
                            {:task-name (str "Sequential Task " i)
                             :duration  300}))
          ...
          (let [async-start (System/currentTimeMillis)
            ...
            (let [async-time (- (System/currentTimeMillis) async-start)]
              ...
              (println (format "Speedup: %.1fx"
                               (double (/ sequential-time async-time)))))))))))

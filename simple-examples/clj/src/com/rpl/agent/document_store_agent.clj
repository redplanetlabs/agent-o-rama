...
(aor/defagentmodule DocumentStoreModule
  ...
  ;; Key: String (user-id), Fields: name (String), email (String), age (Long),
  ;; preferences (Object)
  (aor/declare-document-store topology
  ...
  ;; Key: String (product-id), Fields: title (String), price (Double), category
  ...
  (->
    ...
    (aor/node
     ...
     "update-product"
     (fn [agent-node
          {:keys [user-id profile-updates product-id product-updates]}]
       (let [profiles-store (aor/get-store agent-node "$$user-profiles")]
         ...
         (let [name        (store/get-document-field profiles-store
                                                     user-id
                                                     "name")
               email       (store/get-document-field profiles-store
                                                     user-id
                                                     "email")
               age         (store/get-document-field profiles-store
                                                     user-id
                                                     "age")
               preferences (store/get-document-field profiles-store
                                                     user-id
                                                     "preferences")]
     ...
     (fn [agent-node {:keys [user-id user-profile product-id product-updates]}]
       (let [products-store (aor/get-store agent-node "$$products")]
         ...
         (when (:title product-updates)
           (store/put-document-field! products-store
                                      product-id
                                      "title"
         ...
         (let [title    (store/get-document-field products-store
                                                  product-id
                                                  "title")
               price    (store/get-document-field products-store
                                                  product-id
                                                  "price")
               category (store/get-document-field products-store
                                                  product-id
                                                  "category")
               metadata (store/get-document-field products-store
                                                  product-id
                                                  "metadata")]

           (println
            (format "Updated product %s: title=%s, price=%.2f, category=%s"
                    product-id
                    title
                    (or price 0.0)
                    category))
    ...
    (aor/node
     ...
     (fn [agent-node {:keys [user-id user-profile product-id product-data]}]
       (let [profiles-store (aor/get-store agent-node "$$user-profiles")
         ...
         (let [user-age      (store/get-document-field profiles-store
                                                       user-id
                                                       "age")
               product-price (store/get-document-field products-store
                                                       product-id
                                                       "price")
  ...
  (with-open [ipc (rtest/create-ipc)]
    ...
    (let [manager (aor/agent-manager ipc
      ...
      (let [result1 (aor/agent-invoke
        ...
        (println "Result 1:")
        (println "  User:"
                 (select-keys (:user-profile result1) [:name :email :age]))
        (println "  Product:"
                 (select-keys (:product-data result1)
      ...
      (let [result2 (aor/agent-invoke
                     ...
                     {:user-id         "user123"
                      ...
                      :product-id      "prod456"
                      :product-updates {:metadata {:specs
                                                   "16GB RAM, 512GB SSD"}}})] ; Merge
                                                                              ; metadata
        (println "Result 2:")
        (println "  User age updated to:" (:user-age result2))
        (println "  Product metadata:"
                 (get-in result2 [:product-data :metadata]))
      ...
      (let [result3 (aor/agent-invoke
        ...
        (println "Result 3:")
        (println "  User:"
                 (select-keys (:user-profile result3) [:name :email :age]))

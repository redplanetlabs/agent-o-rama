    ...
    (with-open [ipc (rtest/create-ipc)]
      ...
      (let [manager (aor/agent-manager ipc (rama/get-module-name DocumentStoreModule))
        ...
        (testing "updates individual fields independently"
          (let [result (aor/agent-invoke agent
                                         {:user-id "test-user"
                                          :profile-updates {:age 30}  ; Only update age
                                          :product-id "test-product"
                                          :product-updates {:price 150.0}})] ; Only update price
            (is (= 30 (:user-age result)))
            (is (= "Test User" (get-in result [:user-profile :name]))) ; Name unchanged
            (is (= 150.0 (:product-price result)))
            (is (= "Test Product" (get-in result [:product-data :title]))) ; Title unchanged
            (is (= "premium" (:recommendation result))))) ; Should be premium now

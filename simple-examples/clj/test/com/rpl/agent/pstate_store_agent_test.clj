(ns com.rpl.agent.pstate-store-agent-test
  (:require
   [clojure.test :refer [deftest testing is]]
   [com.rpl.agent-o-rama :as aor]
   [com.rpl.rama :as rama]
   [com.rpl.rama.test :as rtest]
   [com.rpl.agent.pstate-store-agent :refer [PStateStoreModule]]))
      ...
      (let [manager (aor/agent-manager ipc (rama/get-module-name PStateStoreModule))
        ...
        (testing "adds second employee and calculates metrics"
          (let [result (aor/agent-invoke agent
            ...
            (is (= #{"Test Employee" "Second Employee"} (set (:all-company-employee-names result))))))
        
        (testing "updates existing employee"
          (let [result (aor/agent-invoke agent
                                         {:company-id "test-corp"
                                          :dept-id "tech"
                                          :employee {:id "emp1"  ; Same ID - should update
                                                     :name "Test Employee"

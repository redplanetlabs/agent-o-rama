...
(deftest forking-agent-test
  (testing "ForkingAgent example produces expected forking behavior"
    (with-open [ipc (rtest/create-ipc)]
      ...
      (let [manager (aor/agent-manager ipc (rama/get-module-name ForkingAgentModule))
        ...
        (testing "synchronous forking works correctly"
          (let [base-invoke (aor/agent-initiate agent
                ...
                fork-result (aor/agent-fork agent base-invoke
                                            {"calculate" [{:original-input {:base-value 8
                                                                             :multiplier 3}
                                                           :processed-value 24}]})]
        ...
        (testing "async forking works correctly"
          (let [base-invoke (aor/agent-initiate agent
                                                {:base-value 2
                                                 :multiplier 3})
                _ (aor/agent-result agent base-invoke) ; Wait for completion
                fork-invoke (aor/agent-initiate-fork agent base-invoke
        ...
        (testing "fork with invalid data"
          (let [base-invoke (aor/agent-initiate agent
                ...
                _ (aor/agent-result agent base-invoke) ; Wait for completion
                fork-result (aor/agent-fork agent base-invoke
                                            {"validate" [{:original-input {:base-value 0
                                                                            :multiplier 0}
                                                          :processed-value 0
                                                          :squared 0
                                                          :halved 0.0}]})]
            ...
            (is (= false (:valid? fork-result))) ; Not valid because processed-value is not positive
            )))))))
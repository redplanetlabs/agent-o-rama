...
(defn- check-for-failure!
  ...
  (let [{:keys [fail-at-node retries-before-success run-id]} control-params]
...
(aor/defagentmodule E2ETestAgentModule
  ...
  (aor/declare-evaluator-builder
   ...
   (fn [params]
     (let [fail-trigger (:fail_if_contains params)]
       (fn [fetcher input ref-output output]
      ...
      (aor/node
       ...
       (fn [agent-node params]
         (let [control-params (get params :control-params {})
           ...
           (if (:long-node-names? control-params)
      ...
      (aor/node
       ...
       (fn [agent-node params]
         (let [control-params (get params :control-params {})]
           ...
           (aor/result! agent-node (:output-value control-params)))))))

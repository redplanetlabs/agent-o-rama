 ...
 *
 * This module implements a conversational agent that can reason about user
 * queries and take actions using web search tools. It demonstrates the ReAct
 * pattern where the agent alternates between reasoning about what to do and
 * taking actions.
 */
public class ReActModule extends AgentsModule {
    ...
    protected void defineAgents(AgentsTopology topology) {
        ...
        topology.declareAgentObject("openai-api-key",
                                    System.getenv("OPENAI_API_KEY"));

        // Declare Tavily API key from environment
        topology.declareAgentObject("tavily-api-key",
                                    System.getenv("TAVILY_API_KEY"));

        // Declare OpenAI chat model builder
        topology.declareAgentObjectBuilder("openai",
        ...
        topology.declareAgentObjectBuilder("tavily",
                                           (AgentObjectSetup setup) -> {
        ...
        topology.newAgent("ReActAgent")
            .node("chat", "chat", new ChatNodeFunction());

...
public class TestModules {
  public static class BasicToolsOpenAIAgent extends AgentsModule {
    public static List<ToolInfo> TOOLS =
        Arrays.asList(
            ToolInfo.create(
          ...
          .aggStartNode(
              ...
              (AgentNode node) -> {
                node.emit(
                    "model",
                    "a",
                    "What is five added to three? Use a tool call to answer the question.");
                node.emit(
                    "model",
                    "m",
                    "What is six multiplied by eight? Use a tool call to answer the question.");
                return null;
              })
          .node("model", "agg", BasicToolsOpenAIAgent::doModelCall)
          .aggNode(
              "agg",
              null,
              BuiltIn.MAP_AGG,

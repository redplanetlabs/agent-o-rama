...
public interface AgentManager extends IFetchAgentClient, Closeable {
   ...
   * @param categories set of category strings that humans can select from. Must be non-empty.
   */
  void createCategoricalHumanMetric(String name, String description, Set<String> categories);
   ...
   * @param max maximum allowed value (inclusive). Must be greater than or equal to min.
   */
  void createNumericHumanMetric(String name, String description, int min, int max);

  /**
   * Removes a human metric from the system.
   *
   * @param name name of the human metric to remove
   */
  void removeHumanMetric(String name);

...
(defn add-to-dataset-action-builder-fn
  ...
  (let [input-template (h/parse-json-path-template inputJsonPathTemplate)
        output-template (h/parse-json-path-template outputJsonPathTemplate)
...
(defn merge-action-params-with-defaults
  "Merges action params with defaults from options. User-provided params override defaults."
  [params options]
  (let [param-defs (get options :params {})
        defaults (into {}
...
(deframaop find-qualified-offsets-and-run-unlimited
  ...
  (<<with-substitutions
   ...
   (merge-action-params-with-defaults *action-params *options :> *merged-params)
   (completable-future> (build-action-fn *builder-fn *merged-params) :> *action-fn)

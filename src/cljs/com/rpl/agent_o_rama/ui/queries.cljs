...
(defhook use-sente-query 
  ...
  [{:keys [query-key sente-event timeout-ms enabled?]
  ...
  (let [state-path (into [:queries] query-key)
        ...
        ;; Create a stable key string from query-key elements
        ;; Join the query-key vector into a string for use as dependency
        query-key-str (str (vec query-key))]
    
    (uix/use-effect
     (fn []
       ;; Fetch if connected and enabled
       ...
       ;; Cleanup function
       (constantly nil))
     [connected? query-key-str enabled?]) ;; Re-run when connection, query-key, or enabled flag changes

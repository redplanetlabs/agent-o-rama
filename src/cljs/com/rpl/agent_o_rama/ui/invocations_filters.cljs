...
(def filters-path [:ui :invocations-filters])
...
(state/reg-event :invocations-filters/init
                 (fn [db filters-encoded]
                   (let [applied (applied-filters-from-url filters-encoded)
                     ...
                     (s/multi-path
                      [(into filters-path [:draft]) (s/terminal-val draft)]
                      [(into filters-path [:applied]) (s/terminal-val applied)]
                      [(into filters-path [:open-editor]) (s/terminal-val nil)]
                      [(into filters-path [:active-filter-types]) (s/terminal-val active-types)]))))

(state/reg-event :invocations-filters/update-draft
                 (fn [db update-fn]
                   [(into filters-path [:draft]) (s/terminal update-fn)]))
...
(state/reg-event :invocations-filters/apply
                 (fn [db]
                   (when-let [open-editor (s/select-one (into filters-path [:open-editor]) db)]
                     (let [draft (s/select-one (into filters-path [:draft]) db)
                       ...
                       (s/multi-path
                        [(into filters-path [:draft]) (s/terminal-val next-draft)]
                        [(into filters-path [:applied]) (s/terminal-val next-applied)]
                        [(into filters-path [:open-editor]) (s/terminal-val nil)]
                        [(into filters-path [:active-filter-types]) (s/terminal-val active-types)])))))

(state/reg-event :invocations-filters/clear-type
                 (fn [db filter-type]
                   (let [draft (s/select-one (into filters-path [:draft]) db)
                         next-draft (case filter-type
                                      ...
                                      :feedback (-> draft
                                                    (s/setval [:feedback-current] {:metric-name ""
                                                                                  ...
                                                                                  :value ""
                                                                                  :source "any"})
                                                    (s/setval [:feedback-metrics] [{:metric-name ""
                                                                                    :metric-type "numeric"
                                                                                    :match-any-value? false
                                                                                    :allowed-values []
                                                                                    :comparator "<="
                         ...
                         next-applied (build-filter-map next-draft)
                         active-types (derive-active-filter-types next-applied)
                         open-editor (s/select-one (into filters-path [:open-editor]) db)
                         close-editor? (= filter-type (:filter-type open-editor))]
                     (s/multi-path
                      [(into filters-path [:draft]) (s/terminal-val next-draft)]
                      [(into filters-path [:applied]) (s/terminal-val next-applied)]
                      [(into filters-path [:active-filter-types])
                       (s/terminal (fn [types] (vec (remove #(= % filter-type) types))))]
                      [(into filters-path [:open-editor])
                       (s/terminal (fn [v] (if close-editor? nil v)))]))))

(state/reg-event :invocations-filters/add-type
                 (fn [db filter-type]
                   (let [active-types (s/select-one (into filters-path [:active-filter-types]) db)
                         draft (s/select-one (into filters-path [:draft]) db)
                         ...
                         open-editor (case filter-type
                                      ...
                                      {:chip-id (str "singleton-" (name filter-type))
                                       ...
                                       :mode :edit})]
                     (s/multi-path
                      [(into filters-path [:active-filter-types]) (s/terminal-val next-types)]
                      [(into filters-path [:draft]) (s/terminal-val next-draft)]
                      [(into filters-path [:open-editor]) (s/terminal-val open-editor)]))))

(state/reg-event :invocations-filters/open-chip
                 (fn [db {:keys [chip-id filter-type node-name node-idx feedback-idx]}]
                   (let [open-editor (s/select-one (into filters-path [:open-editor]) db)
                         applied (s/select-one (into filters-path [:applied]) db)
                         draft (s/select-one (into filters-path [:draft]) db)]
                       ...
                       (let [next-draft (case filter-type
                                          ...
                                          (let [metrics (or (:feedback-metrics applied) [])
                                            ...
                                            (s/setval [:feedback-current]
                                                     ...
                                                     draft))
                                          draft)
                         open-editor {:chip-id chip-id :filter-type filter-type :mode :edit
                                      :node-idx node-idx :node-name node-name :feedback-idx feedback-idx}]
                       (s/multi-path
                        [(into filters-path [:draft]) (s/terminal-val next-draft)]
                        [(into filters-path [:open-editor]) (s/terminal-val open-editor)]))))))

(state/reg-event :invocations-filters/remove-chip
                 (fn [db {:keys [filter-type node-idx feedback-idx]}]
                   (cond
                     ...
                     (state/dispatch [:invocations-filters/clear-type :source])

                     (or (= filter-type :latency) (= filter-type "latency"))
                     (state/dispatch [:invocations-filters/clear-type :latency])

                     (or (= filter-type :error) (= filter-type "error"))
                     (state/dispatch [:invocations-filters/clear-type :error])
...
(defui filter-bar [{:keys [node-options feedback-metric-options feedback-metric-options-by-name
                           feedback-metric-option-names]}]
  (let [draft (or (state/use-sub (into filters-path [:draft])) default-draft-filters)
        applied (or (state/use-sub (into filters-path [:applied])) default-applied-filters)
        open-editor (state/use-sub (into filters-path [:open-editor]))
        active-types (or (state/use-sub (into filters-path [:active-filter-types])) [])
        ...
        add-items (map (fn [ft]
                         {:key (name ft)
                          ...
                          :on-select #(state/dispatch [:invocations-filters/add-type ft])})
                       filter-type-order)
        update-draft! #(state/dispatch [:invocations-filters/update-draft %])
        apply! #(state/dispatch [:invocations-filters/apply])
        open-chip! #(state/dispatch [:invocations-filters/open-chip %])
        remove-chip! #(state/dispatch [:invocations-filters/remove-chip %])]
                                     ...
                                     (update-draft! (fn [prev]
                                                      (assoc prev :feedback-current
                                                             {:metric-name selected-name
                                                              ...
                                                              :comparator "<="
                                                              :value ""
                                                              :source (s/select-one [:feedback-current :source] prev "any")}))))}

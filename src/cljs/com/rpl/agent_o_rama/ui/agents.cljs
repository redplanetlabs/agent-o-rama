(ns com.rpl.agent-o-rama.ui.agents
  (:require
   ...
   [com.rpl.agent-o-rama.ui.invocations-filters :as invocations-filters]
...
(defui invocations []
  (let [{:keys [module-id agent-name]} (state/use-sub [:route :path-params])
        ...
        applied-filters (or (state/use-sub [:ui :invocations-filters :applied])
                            invocations-filters/default-applied-filters)
        ...
        _init-filters (uix/use-effect
                      (fn []
                        (state/dispatch [:invocations-filters/init filters-query-param])
                        js/undefined)
                      [filters-query-param])
        _sync-filters-to-url (uix/use-effect
                              (fn []
                                (when (and module-id agent-name)
                                  (let [encoded (invocations-filters/encode-filters-param applied-filters)
    ...
    ($ :div.p-4.space-y-4
       ($ invocations-filters/filter-bar
          {:node-options node-options
           :feedback-metric-options feedback-metric-options
